...
def Ecran_Chargement() :
	...
	class Ecran(Thread) :
		...
		def run(self) :
			#Affiche le texte en fade in
			for i in range(255,0,-4):
				fenetre.blit(texte, (position_texte))
				fenetre.fill((i,i,i), special_flags=BLEND_RGB_SUB)
				sleep(0.015)
				pygame.display.flip()
			#Garde le texte affiché tant que self.affichage est vrai, ou tant qu'on ne clique pas sur la croix
			while self.affichage :
				for event in pygame.event.get() :
					if event.type == QUIT :
						quit()
			#Quand l'affichage se termine, efface le texte en fade out
			Disparition_Fondu(0.005)

	Ecran = Ecran()

	class Chargement(Thread) :
		...
		def run(self) :
			for carte in Paquet :
				texteCarte = (" ").join(carte)
				if not tuple(carte) in dicoDesCartes :
					dicoDesCartes[tuple(carte)] = pygame.image.load("Pics/Cartes/%s.png" %texteCarte)
			Ecran.affichage = False
...
if __name__ == "__main__" :
	...
	#Fait apparaitre l'ecran titre et attend. Renvoie les nouvelles largeurs et hauteurs dns le cas d'un redimensionnement
	...
	#Initialiisation des images et des sons
	laugh = pygame.mixer.Sound("Sounds/ZSS_Ghirahim_Laugh_SlowMotion.wav")
	...
	Background = choice(["Pics/BackgroundJeu.png", "Pics/BackgroundJeu2.png", "Pics/BackgroundJeu3.png", "Pics/BackgroundJeu4.png"])
	...
	pygame.mixer.music.load("Sounds/Musics/In Game.wav")
